package com.github.pokeclash.cobblewand.platform.fabric;
...
public class CobbleWandPermissionPlatformImpl {
    public static boolean hasPermission(ServerPlayer player, String node) {
        Boolean fabricApiResult = hasFabricPermissionsApi(player, node);
    ...
    private static Boolean hasFabricPermissionsApi(ServerPlayer player, String node) {
        try {
            Class<?> permissionsClass = Class.forName("me.lucko.fabric.api.permissions.v0.Permissions");
            Method check = permissionsClass.getMethod("check", net.minecraft.commands.CommandSourceStack.class, String.class, boolean.class);
            return (Boolean) check.invoke(null, player.createCommandSourceStack(), node, false);
        } catch (ReflectiveOperationException | IllegalArgumentException | SecurityException | LinkageError ignored) {
            return null;
        }
    }
